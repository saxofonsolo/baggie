import{r as l,R as u}from"./index-4g5l5LRQ.js";function V(o,a=null){const g=l.useRef(a);return l.useEffect(()=>{if(o)if(typeof o=="function"){o(g.current);return}else o.current=g.current}),g}const _=(o,a)=>l.useMemo(()=>{if(!a)return[];const{ranges:g,regexes:i,strings:p}=(Array.isArray(a)?typeof a[0]=="number"?[a]:a:[a]).reduce((e,t)=>(typeof t=="string"?e.strings.push(t):Array.isArray(t)&&typeof t[0]=="number"&&typeof t[1]=="number"?e.ranges.push(t):t instanceof RegExp&&e.regexes.push(t),e),{ranges:[],regexes:[],strings:[]});if(!g.length&&!i.length&&!p.length)return[];const f=[];for(let e=0;e<p.length;e++){let t=-1;for(;(t=o.indexOf(p[e],t+1))>=0;)f.push([t,t+p[e].length])}const m=[];for(let e=0;e<i.length;e++){const t=[];if(i[e].global){let s=-1,c;for(;(c=i[e].exec(o))!==null&&(!t.length||c.index>t[s][0]);)t.push([c.index,c.index+c[0].length]),s++}else{const s=i[e].exec(o);s&&t.push([s.index,s.index+s[0].length])}m.push(...t)}const y=[...g,...f,...m];if(!y.length)return[];const h=y.sort((e,t)=>e[0]-t[0]),r=[h[0]];for(let e=1;e<h.length;e++){const[t,s]=h[e],c=r[r.length-1];c[1]>=t?c[1]=Math.max(c[1],s):r.push(h[e])}return[o.slice(0,r[0][0]),...r.map(([e,t],s)=>u.createElement(u.Fragment,{key:`${e}:${t}`},u.createElement("mark",null,o.slice(e,t)),r[s+1]?o.slice(t,r[s+1][0]):void 0))]},[o,a]);(function(){try{if(typeof document<"u"){var o=document.createElement("style");o.appendChild(document.createTextNode(`._highlightArea_4i7ca_1 {
